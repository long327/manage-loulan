      ...
      <div class="lis-container" v-loading="listLoading">
        ...
        <el-row>
          <el-col
            ...
            :key="index"
            style="margin: 0px 5px 30px 0px; width:310px"
          >
            <el-card
              :body-style="{
                padding: '0px',
        ...
        <div class="submit">
          <el-row type="flex" justify="center">
            <el-col :span="12" style="text-align: right; margin-right: 50px">
              <el-button round @click="cancelPics"
                >取消</el-button>
            </el-col>
            <el-col :span="12" style="margin-left: 50px">
              <el-button round type="primary" @click="determinePics">确定</el-button>
            </el-col>
          </el-row>
        </div>
      </div>
...
export default {
  ...
  watch: {
    fileListAudio(nv, ov) {
      this.diaTitle = this.fileListAudio.length;
    }
  },
  methods: {
    openDialog() {
    ...
    clickvalue(item, flag) {
      ...
      if (this.fileListAudio.length > total) {
        if (flag) {
        } else {
          this.$message({
            message: "音频最多是1个",
      ...
      this.picsList.forEach((val) => {
        if (val.id == item.id && flag == false) {
          val.checked = true;
          this.fileListAudio.push(val);
        }
        if (val.id == item.id && flag) {
          val.checked = false;
          this.fileListAudio.forEach((item, idx) => {
            if (item.id == val.id) {
              item.checked = false;
              this.$emit('chFileListAudio', this.arrSlice(this.fileListAudio, idx))
            }
          });
        }
      });
    ...
    getList() {
      ...
      getList(this.picslistQuery)
        .then((response) => {
          ...
          response.list.forEach((item) => {
            this.fileListAudio.forEach((val) => {
              if (item.id == val.id) {
...
debugger;
      ...
      if (this.fileListAudio.length > 0) {
        this.$emit('showuploadAudio', 'audio');
      }

      this.$emit('chFileListAudio', this.fileListAudio)
